...
/*
 * This takes a Stereoscopic 3D screenshot of the current scene, and saves it as a .png file.
 * 
 * To use this:
 * 	Place this script on your camera or object at the center of your players view.
 * 	Select a screenshot key.
 * 	Adjust the focalDistance to about you would want it for objects inbetween the Plane visual and the camera to appear to "pop out of the image" in the resulting screenshot.
 * 	Compile, Run, and press that key when you see something you want to capture.
 * 
 * Optional:
 ...
 * 	If you just want to use this script to capture and display a 3D texture directly in game, you can leave the key parameter set to None and call Get3DTextures() whenever you want.
 * 	
 * Notes:
...
namespace VR3D
{
    public class Stereoscopic3DScreeenshot : MonoBehaviour
    {
        ...
        [Header("Other")]
        [Tooltip("Leave this set to None if you dont need a screenshot key.")]
        public KeyCode key;
        ...
        /// </summary>
        private const float frustrumPlaneAlpha = 0.25f;
        ...
        void Update()
        {
            if (key != KeyCode.None)
            {
                ...
                {
                    TakeScreenshot();
                    Debug.Log("Screenshot Taken: Saved to \"" + Application.dataPath + "\".");
                }
            }
        }
        ...
        void OnDrawGizmos()
        {
            if (UnityEditor.Selection.activeObject == gameObject)
            {
                ...
                Vector3 leftCameraPosition = transform.localPosition + transform.right * -((seperation / 2) / 1000);
                Vector3 rightCameraPosition = transform.localPosition + transform.right * ((seperation / 2) / 1000);
                ...
                Gizmos.color = new Color(1.0f, 0, 0, frustrumPlaneAlpha);
                ...
                Gizmos.color = new Color(0, 1.0f, 1.0f, frustrumPlaneAlpha);
        ...
        {
#if !UNITY_WEBPLAYER
            // Capture our texture. 0x0 for now. We resize it later as appropriate to the image format.
            ...
            if (imageFormat == ScreenShotImageFormat.TwoImages)
            {
                // Save the PNG file to disc in the applications directory.
                File.WriteAllBytes(Application.dataPath + "/../" + Application.productName + " - " + time.ToString(dateFormat) + "-Left.png", bytes);
                ...
                // Save the PNG file to disc in the applications directory.
                File.WriteAllBytes(Application.dataPath + "/../" + Application.productName + " - " + time.ToString(dateFormat) + "-Right.png", bytes);
            }
            else
            {
                // Save the PNG file to disc in the applications directory.
                File.WriteAllBytes(Application.dataPath + "/../" + Application.productName + " - " + time.ToString(dateFormat) + ".png", bytes);
            }
#else
        Debug.LogWarning("Swtich build out of WebPlayer for saving screenshots to work.");
#endif
        }
        ...
        public void Get3DTextures(Texture2D texture, Texture2D texture2 = null)
        {
            ...
            leftCameraGO.transform.localPosition = transform.localPosition + transform.right * -((seperation / 2) / 1000);
            rightCameraGO.transform.localPosition = transform.localPosition + transform.right * ((seperation / 2) / 1000);
        ...
        public static Texture2D TopBottomToInterlacedTexture(Texture2D sourceTexture)
        {
            ...
            // Save the PNG file to disc in the applications directory.
            //File.WriteAllBytes(Application.dataPath + "/../" + Application.productName + " - " + 1 + ".png", bytes);
        ...
        /// <returns>A Side-by-Side format Stereoscopic 3D texture.</returns>
        public static Texture2D AnaglyphToSBSTexture(Texture2D sourceTexture, bool makeMonochrome = false)
        {
